...
class Task
{
    ...
    {
        $method_query = getQuery($method, 'tasks.task.add', [
            'fields' => [
                'TITLE' => $task['title'],
    ...
    {
        $task = getQuery($method, 'tasks.task.get', [
            'taskId' => $tasks,
            'select' => [
    ...
    public function uploadFile($method, $folder_id, $file)
    {
        $file_upload_task = getQuery($method, 'disk.folder.uploadfile', [
            'id' => $folder_id,
            'data' => [
                'NAME' => $file['result']['NAME']
            ],
            'fileContent' => [$file['result']['NAME'], base64_encode($file)],
